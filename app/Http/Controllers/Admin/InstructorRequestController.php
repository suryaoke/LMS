...
class InstructorRequestController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $instructorRequest = User::where('approve_status', 'pending')->orWhere('approve_status', 'rejected')->get();
        ...
        switch ($instructor_request->approve_status) {
            case 'approved':
                if (config('mail_queue.is_queue')) {
                    Mail::to($instructor_request->email)->queue(new InstructorApprovedMail());
                } else {
                    Mail::to($instructor_request->email)->send(new InstructorApprovedMail());
                }
                break;
            case 'rejected';
